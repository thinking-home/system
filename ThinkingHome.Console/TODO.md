...
- [x] разобраться с тестом `TelegramBotPlugin.CommandCanContainsProperSymbols`: падает (и падал до правок 2026-08-20) — тест ожидает кириллицу в командах, а `CommandRegex` пропускает только `[a-z0-9-_]`; починить либо регулярку, либо тест (решение 2026-08-30: команды — только латиница; тест и документация приведены к коду)
