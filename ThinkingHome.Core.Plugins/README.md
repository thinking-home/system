...
в ресурсах плагина и во время его работы он автоматически получит строки на нужном языке. Язык выбирается в [настройках приложения](#Настройки)).
...
и по одному файлу для каждого дополнительного языка (например, для русского). Названия файлов должны совпадать
с названием плагина. Файлы для дополнительных языков должный иметь суффикс, соответствующий [коду их языка](http://www.csharp-examples.net/culture-names). 
