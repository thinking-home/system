...
Динамический ресурс `/api/telegram-chat-list/web-api/list` SHALL без параметров возвращать список всех записей о чатах, сохранённых `plugins/telegram-chat-list/chat-tracking`, упорядоченный по убыванию `date`: запись с самой поздней датой — первой. Каждая запись в ответе SHALL представляться объектом с полями `id`, `login`, `chatId`, `firstName`, `lastName` и `date`, соответствующими одноимённым (с точностью до регистра) полям хранимой записи о чате.
...
- **GIVEN** сохранено несколько записей о чатах с разными значениями даты
- **WHEN** выполняется запрос `/api/telegram-chat-list/web-api/list`
- **THEN** записи в ответе идут в порядке убывания `date`: первой — запись с самой поздней датой
