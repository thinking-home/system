...
Событие возникает при получении каждого входящего сообщения — в отличие от обработчиков `[TelegramMessageHandler]`, в том числе для сообщений из групповых чатов и от пользователей, которых нет в `authorizedLogins`.
...
public class MyPlugin : PluginBase
{
    ...
    {
        telegram.OnMessageReceived += msg => Logger.LogInformation("chat {ChatId}", msg.Chat.Id);
    }
}
```
...
Обработчики вызываются только для сообщений в личных чатах от пользователей, перечисленных в `authorizedLogins` (см. [конфигурацию](#конфигурация)); сообщения из групповых чатов и от остальных пользователей игнорируются.

В качестве параметра атрибута необходимо указать команду бота, которую следует обрабатывать. Например, если указать значение `"test"`, то ваш метод будет вызван для всех сообщений, которые начинаются с `/test`.

Команды бота могут содержать английские буквы, цифры, символы "дефис" `-` и "подчеркивание" `_`. В начале команды должен быть слэш `/`.
