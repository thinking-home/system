...
Вы можете настраивать параметры подключения к MQTT брокеру и пользовательские сценарные события, которые генерируются при получении сообщений MQTT. Параметры `host` и `port` можно не указывать — по умолчанию используется `localhost:1883`.
...
- `string topicFilter` — фильтр сообщений: если пришло сообщение в топик, который соответствует этому фильтру, то будет вызван заданный обработчик. В фильтре можно использовать [подстановки](https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901241).
...
В настройках плагина при помощи параметра `scriptEvents` вы можете связать [пользовательские сценарные события](../ThinkingHome.Plugins.Scripts/README.md#пользовательские-события) с сообщениями MQTT.

В качестве значения параметра `scriptEvents` нужно указать объект, ключи которого содержат фильтры топиков MQTT (можно использовать [подстановки](https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901241) `+` и `#` — сообщения сопоставляются с фильтрами на стороне брокера), а значения — имена пользовательских событий, которые будут генерироваться при получении подходящих сообщений.
...
Чтобы подписать сценарий на такое событие, нужна подписка на событие `scripts:user-event` с [фильтром по meta](../ThinkingHome.Plugins.Scripts/README.md#фильтры-подписок): `name` = имя события из конфигурации. Фильтр по `topic` дополнительно позволяет реагировать только на конкретный топик.

В словаре meta события передаются значения:

- `name` - имя события (из конфигурации);
- `topic` - топик, в который пришло сообщение.

Аргументы события:

- `args[0]` - топик, в который пришло сообщение (`string`);
- `args[1]` - полученные данные (`Buffer`).

#### Пример обработчика

```js
var text = args[1].ToUtf8String();

host.log.info('[MQTT MESSAGE] ' + meta.topic + ': ' + text);
```
