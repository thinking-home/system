...
Вы можете настраивать название serial порта, к которому подключен адаптер nooLite. Параметр `portName` обязателен — без него приложение не запустится.
...
- `fMode` (`boolean`) - режим отправки команд:
